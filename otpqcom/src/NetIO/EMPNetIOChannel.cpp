...
namespace emp::network {
    EMPNetIOChannel::EMPNetIOChannel(otpq::network::NodeNetworkConfig netcfg, const EMPNetworkNodeType nodeType)
        : netcfg_(std::move(netcfg)), nodeType_(nodeType) {
        if (nodeType == EMPNetworkNodeType::SERVER)
            setup_server();
        else
            setup_client();
    ...
    EMPNetIOChannel::~EMPNetIOChannel() {
        flush();
        if (stream_) {
            fclose(stream_);
        }
    ...
    void EMPNetIOChannel::nodeListen() {
        if (nodeType_ == EMPNetworkNodeType::SERVER) {
    ...
    void EMPNetIOChannel::set_no_delay() const {
        constexpr int one{1};
        setsockopt(connSocket_, IPPROTO_TCP, TCP_NODELAY, &one, sizeof(one));
    }

    void EMPNetIOChannel::set_delay() const {
        const int zero = 0;
        setsockopt(connSocket_, IPPROTO_TCP, TCP_NODELAY, &zero, sizeof(zero));
    }

    void EMPNetIOChannel::flush() const {
        fflush(stream_);
    }

    void EMPNetIOChannel::send_data_internal(const void *data, std::size_t len) {
        std::size_t sent = 0;
    ...
    void EMPNetIOChannel::recv_data_internal(void *data, std::size_t len) {
        if (hasSent_) {
            fflush(stream_);
        }
        hasSent_ = false;
    ...
    void EMPNetIOChannel::setup_server() {
        sockaddr_in serv{};
    ...
    void EMPNetIOChannel::setup_client() {
        connSocket_ = socket(AF_INET, SOCK_STREAM, 0);
        if (connSocket_ < 0)
